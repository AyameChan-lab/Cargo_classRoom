...
fn api_serve(db_pool: Arc<PgPoolSquad>) -> Router {
    Router::new()
        .nest("/auth", routers::authentication::routes(db_pool.clone()))
        .nest("/brawlers", routers::brawlers::routes(db_pool.clone()))
        .nest(
            "/missions-management",
            routers::mission_managment::route(db_pool.clone()),
        )
        .nest(
            "/mission",
            routers::mission_operation::routes(db_pool.clone()),
        )
        .nest("/view", routers::mission_viewing::routes(db_pool.clone()))
        .nest("/crew", routers::crew_operation::routes(db_pool))
        .nest("/error", routers::buggy_routes::routes())
